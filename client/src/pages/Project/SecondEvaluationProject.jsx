...
const SecondEvaluationProject = (props) => {
    ...
    const setSecondEvaluationProject = useStoreActions(actions => actions.project.setSecondEvaluationProject);
    const evaluation = project.second_evaluation?.filter(x => x.created_by == user.id)[0]
    ...
    const onSubmit = (secondEvaluation) => {
        ...
        setSecondEvaluationProject(evaluation).then(result => {
            ...
            setTimeout(() => history.push(`/dashboard/jurado/premiatoria`), 5000)
        }).catch((err) => {
