...
const SwornDashboard = () => {
    ...
    if (user.type === 'swornpremiatoryend') {
        projectsFilter = projects.filter(project => project.sworn_premiatory_end.includes(user.id));
        ...
        return (
            <TableRow>
                <Modal
                    open={openModal}
                    onClose={handleClose}>
                    <DetailEvaluation project={project} evaluation={project.three_evaluation?.filter(x => x.created_by == user.id)[0]} />
                </Modal>
                ...
                <TableCell align="center">
                    <IfElse condition={project.three_evaluation?.filter(x => x.created_by == user.id).length > 0}
                        True={
                            <IfElse condition={project.three_evaluation?.filter(x => x.created_by == user.id)[0]?.status === 'CONCLUDED'}
                                True={
                                    <Save>Salvo</Save>
                                }
                                False={
