...
import Vencedores from './vencedores.json';
...
const ProjectDashboard = () => {
    ...
    const defineSwornPremiatoryToProject = useStoreActions(actions => actions.project.defineSwornPremiatoryEndToProject);
    ...
    useEffect(() => {
        getAllUsers()
            .then((res) => {
                allUserSworn = (res.data).filter(user => user.type === 'swornpremiatoryend');
                getAllProjects('');
            }).catch((err) => {
                console.log(err);
            });
    ...
    const projects = allProjects.map(project => {
        return {
            ...
            sworn_premiatory_end: project.sworn_premiatory_end || [],
            three_evaluation: project.three_evaluation   || [],
        }
    })
            ...
            <Grid container spacing={3}>
                <Grid item xs={12}>
                    <Title style={{ textAlign: 'left', fontSize: 24, marginBottom: 20, marginTop: 30 }}>
                        {'LISTA DE PROJETOS'} {`(${Vencedores.Contatos.length})`}
                    </Title>
                </Grid>
                ...
                <Grid item xs={12}>
                    <TableContainer component={Paper}>
                        <Table>
                            ...
                            <TableBody>
                                {Vencedores.Contatos.map(y => {
                                    const row = projects.filter(x => x.payment_identifier === y['Codigo'])
                                    if (row.length > 0)
                                    return <RowProject key={row[0]._id} project={row[0]} />
                                })}
                            </TableBody>
                        </Table>
                    </TableContainer>
    ...
    function RowProject(props) {
        ...
        useEffect(() => {
            if (project.sworn_premiatory_end.length !== 0)
                setSworn(project.sworn_premiatory_end);
        }, []);

        const defineSworn = (event) => {
            const swornList = event.target.value;
            defineSwornPremiatoryToProject({ id: project.id, sworn_premiatory_end: swornList.filter(x => x !== "nenhum") }).then((res) => {
