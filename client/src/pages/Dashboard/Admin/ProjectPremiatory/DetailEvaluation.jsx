...
const DetailFirstEvaluation = (props) => {
  ...
  function UserList(props) {
    const { user } = props;
    const projectCurrent = project.second_evaluation?.filter(x => x.created_by == user._id)[0];
    ...
    return (
      <TableRow>
        ...
        <TableCell align="center">
          <IfElse condition={project.second_evaluation?.filter(x => x.created_by == user._id).length > 0}
            True={
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Table>
              ...
              <TableBody>
                {userSworn.filter(x => project.sworn_premiatory.includes(x._id)).map(user => (
                  <UserList user={user} />
                ))}
              </TableBody>
            </Table>
